package restforyou.com.tofsologia;

public class ToDoStuff {
    //todo known issues:
    //1. picture made on scan activity automatically rotate picture on 90 degrees whats makes text unrecognizable
    //2. back button from textRecognitionActivity when photo was chosen leads to crash

    //todo mvp:
    //1. make picture
    //2. recognize picture
    //3. save recognised text

    // using last version of butterKnife force to migrate to androidX
    // http://jakewharton.github.io/butterknife/
    // https://stackoverflow.com/questions/51793345/android-material-and-appcompat-manifest-merger-failed


}
